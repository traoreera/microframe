...
class TemplateManager:
    ...
    def __init__(
        ...
        logger.info(
            f"TemplateManager initialized: {self.directory} (auto_reload={auto_reload}, cache={enable_cache})"
        )
    ...
    def _register_filters(self):
        ...
        filters = {
            ...
            # Listes
            "join_and": lambda items: (
                ", ".join(items[:-1]) + " et " + items[-1]
                if len(items) > 1
...
def get_templates(directory: str = "templates", debug: bool = False) -> TemplateManager:
    """
    ...
    if _template_manager is None:
        _template_manager = TemplateManager(
            directory=directory, debug=debug or os.getenv("DEBUG", "false").lower() == "true"
        )
...
def configure_templates(
    directory: str = "templates", auto_reload: bool = None, enable_cache: bool = None, **kwargs
) -> TemplateManager:
    """
    ...
    global _template_manager
    _template_manager = TemplateManager(
        directory=directory, auto_reload=auto_reload, enable_cache=enable_cache, **kwargs
    )
    return _template_manager
