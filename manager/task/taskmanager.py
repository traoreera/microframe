...
class TaskManager:
            ...
            for job in list_jobs:
                try:
                    if job.get("activate", True):
                        ...
                        if response is True:
                            successful_jobs += 1
                            logger.info(f"Tâche '{job.get('name', 'unknown')}' ajoutée avec succès")
                        else:
                            failed_jobs += 1
                            logger.error(
                        ...
                        logger.info(f" Tâche '{job.get('name', 'unknown')}' désactivée, ignorée")
    ...
    def _add_job(self, job: dict):
        ...
        logger.debug(f"Configuration de la tâche '{job_name}' avec l'intervalle '{job_interval}'")
        ...
        try:
            ...
            if job["interval"] == "interval":
                ...
                logger.debug(f"Tâche '{job_name}' configurée avec intervalle de {minutes} minutes")
