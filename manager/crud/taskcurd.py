...
class ModuleRuntimeManager:
    ...
    def get_all_python_modules(self):
        ...
        return [f.stem for f in self.module_dir.glob("*.py") if not f.name.startswith("__")]
    ...
    def extract_metadata(self, module_name: str) -> dict:
        ...
        try:
            ...
            return getattr(module, "metadata", {}) if hasattr(module, "metadata") else {}
    ...
    def register_new_modules(self):
        ...
        existing_names = [
            task.module for task in self.db.query(TaskModel).with_entities(TaskModel.module).all()
        ]
        ...
        logger.info(f"Modules enregistrés : {[m.title for m in self.get_enabled_modules()]}")
    ...
    def start_module(
        ...
        if mod := self.db.query(TaskModel).filter(TaskModel.module == module_name).first():
