...
class Loader(Repository):
            ...
            try:
                ...
                for mod in loaded_plugins:
                    try:
                        ...
                        self.logger.error(f"❌ Erreur include_router({mod.__name__}): {e}")

                # Forcer la mise à jour du schéma OpenAPI
                self.app.openapi_schema = None
                openapi = self.app.openapi()
                self.logger.debug(f"✅ OpenAPI regénéré ({len(openapi.get('paths', {}))} routes)")

                # Re-synchroniser la table des routes
                if hasattr(self.app.router, "routes"):
                    self.app.router.routes = list(self.app.routes)

                self.logger.info("✅ Routes FastAPI et Swagger synchronisées après startup.")

            except Exception as e:
                self.logger.error(f"❌ Échec du rechargement des plugins au startup: {e}")
    ...
    def _attach_plugins_to_app(self, loaded_plugins: list[Any]) -> None:
        ...
        if before != after:
            self.logger.debug(f"🧹 {before - after} anciennes routes plugin supprimées.")
        ...
        for mod in loaded_plugins:
            try:
                if hasattr(mod, "router") and not _router_already_included(self.app, mod.router):
                    self.app.include_router(mod.router)
